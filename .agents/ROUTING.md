...
**Scope:** Cerulean project (`.agents/skills/` copied from agent-loom + Cerulean domain skills).

---

## Cerulean Domain Layer (highest priority for this repo)

When work is **Cerulean-specific**, prefer these over generic skills:

| Intent | Skill |
|--------|-------|
| Cerulean features, modules, blocks, patches, persistence | `cerulean-project` |
| Deploy Railway + Supabase, migrations, go live | `cerulean-deployment` |
| MCP setup, `cerulean_*` tools, IDE workflow | `cerulean-mcp` |
| Cerulean 11-agent AI system (product agents, not Cursor agents) | `cerulean-project` + `agent-system-architecture` |

`cerulean-*` skills do **not** replace `AGENTS.md` — they route to the right workflow. Full rules stay in `AGENTS.md`.

For Cerulean handoff/state: also read `docs/HANDOFF.md` and `docs/agent-shared-context.md` (parallel to `docs/memory/`).
...
> Using **[this library's skill]** (Cerulean `.agents/skills/`) over **[other skill name]** ([source]).
