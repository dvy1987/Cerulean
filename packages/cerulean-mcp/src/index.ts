...
async function main() {
  ...
  try {
    ...
    logStderr(
      `Connected ✓  document="${check.documentTitle}"  insights=${check.insightCount}  messages=${check.messageCount}`
    );
  } catch (err) {
    logStderr(
      `Warning: could not verify connection at startup — ${err instanceof Error ? err.message : err}`
    );
